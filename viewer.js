...
new ResizeObserver(function(e){var r=e[0];var w,h,dd=window.devicePixelRatio;if(r.devicePixelContentBoxSize){w=r.devicePixelContentBoxSize[0].inlineSize;h=r.devicePixelContentBoxSize[0].blockSize;dd=1}else if(r.contentBoxSize&&r.contentBoxSize[0]){w=r.contentBoxSize[0].inlineSize;h=r.contentBoxSize[0].blockSize}else{w=r.contentRect.width;h=r.contentRect.height}cs=[w*dd,h*dd];if(cam)draw();}).observe(c,{box:'content-box'});
...
// Load PLY (standard 3DGS format, 62 floats per gaussian)
function loadPly(buf,onStep){
var t0=performance.now();
var txt=new TextDecoder('utf-8').decode(buf.slice(0,2000));
var hdrEnd=txt.indexOf('end_header')+'end_header'.length+1;
var N=parseInt(txt.match(/element vertex (\d+)/)[1]);
...
for(var i=0;i<N;i++){
var off=hdrEnd+i*62*4;
var px=dv.getFloat32(off,true),py=dv.getFloat32(off+4,true),pz=dv.getFloat32(off+8,true);
var sh0=dv.getFloat32(off+6*4,true),sh1=dv.getFloat32(off+7*4,true),sh2=dv.getFloat32(off+8*4,true);
var H_END=6+48;
var opacity=dv.getFloat32(off+H_END*4,true);
var sx=Math.exp(dv.getFloat32(off+(H_END+1)*4,true)),sy=Math.exp(dv.getFloat32(off+(H_END+2)*4,true)),sz=Math.exp(dv.getFloat32(off+(H_END+3)*4,true));
var r=dv.getFloat32(off+(H_END+4)*4,true),rx=dv.getFloat32(off+(H_END+5)*4,true),ry=dv.getFloat32(off+(H_END+6)*4,true),rz=dv.getFloat32(off+(H_END+7)*4,true);
...
c[i*3]=.5+SH_C0*sh0;c[i*3+1]=.5+SH_C0*sh1;c[i*3+2]=.5+SH_C0*sh2;
o[i]=1/(1+Math.exp(-opacity));
...
function nr(m,r2){m[r2+0]=-m[r2+0];m[r2+4]=-m[r2+4];m[r2+8]=-m[r2+8];m[r2+12]=-m[r2+12];}
...
T('Downloading model (96 MB)...');
try{
var buf=await new Promise(function(resolve,reject){
var x=new XMLHttpRequest();
x.open('GET','assets/point_cloud.ply',true);
